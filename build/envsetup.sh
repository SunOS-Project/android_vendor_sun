...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^sun_") ; then
        SUN_BUILD=$(echo -n $1 | sed -e 's/^sun_//g')
    else
        SUN_BUILD=
    fi
    export SUN_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/sun/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Sun model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch sun_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/sun-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.sun.device | grep -q "$SUN_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $SUN_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function sunremote()
{
    ...
    local SUN_USER=$(git config --get review.review.sunos.org.username)
    if [ -z "$SUN_USER" ]
    then
        git remote add sun ssh://review.sunos.org:29418/$PFX$PROJECT
    else
        git remote add sun ssh://$SUN_USER@review.sunos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'sun' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/SunOS-Project/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.sun.device | grep -q "$SUN_BUILD");
    then
...
function sungerrit() {
    ...
    local user=`git config --get review.review.sunos.org.username`
...
function sunrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      sunrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.sunos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.sun.device | grep -q "$SUN_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/sun/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $SUN_FIXUP_COMMON_OUT ]; then
