...
"""
Merge script for Sun

 The source directory; this is automatically two folder up because the script
 is located in vendor/sun/build/tools. Other ROMs will need to change this. The logic is
 as follows:
...
MANIFEST_NAME = "snippets/sun.xml"
REPOS_TO_MERGE = {}
REPOS_RESULTS = {}
...
def list_default_repos(is_system):
    ...
    if is_system:
        with open(
            "{0}/.repo/manifests/snippets/system.xml".format(WORKING_DIR)
        ) as system_manifest:
        ...
        with open(
            "{0}/.repo/manifests/snippets/vendor.xml".format(WORKING_DIR)
        ) as vendor_manifest:
...
def merge_manifest(is_system, branch):
    ...
    with open("{0}/.repo/manifests/default.xml".format(WORKING_DIR)) as manifestxml:
        ...
        tree.write("{0}/.repo/manifests/snippets/default.xml".format(WORKING_DIR))
        cpu_count = str(os.cpu_count())
        subprocess.run(
            [
                "repo",
