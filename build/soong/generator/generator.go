...
func init() {
	android.RegisterModuleType("sun_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = sunExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = sunExpandVariables(ctx, dep_file)
	...
	cmd := sunExpandVariables(ctx, String(g.properties.Cmd))
